...
from edeposit.theme.testing import EDEPOSIT_THEME_INTEGRATION_TESTING
from edeposit.theme.testing import EDEPOSIT_THEME_FUNCTIONAL_TESTING
...
class TestSetup(unittest.TestCase):
    ...
    layer = EDEPOSIT_THEME_INTEGRATION_TESTING
    
    def test_css_registry_configured(self):
        portal = self.layer['portal']
        cssRegistry = getToolByName(portal, 'portal_css')
        self.assertTrue("++theme++edeposit.theme/stylesheets/main.css" 
                in cssRegistry.getResourceIds()
            )
        self.assertTrue("++theme++edeposit.theme/stylesheets/iefixes.css"
                in cssRegistry.getResourceIds()
            )
    ...
    def test_theme_configured(self):
        ...
        self.assertEqual(settings.rules, 
                "/++theme++edeposit.theme/rules.xml"
            )
        self.assertEqual(settings.absolutePrefix,
                "/++theme++edeposit.theme"
            )

class TestRendering(unittest.TestCase):
    
    layer = EDEPOSIT_THEME_FUNCTIONAL_TESTING
